...
FileReader::FileReader(const FileReader& fr)
  :  m_input(), m_lastLine(), m_fileName(fr.m_fileName)
{
  m_input.open(m_fileName.c_str());
}

FileReader::FileReader(const string& s) : m_input(), m_lastLine(), m_fileName(s)
{
  m_input.open(m_fileName.c_str());
  if (!m_input.is_open())
...
FileReader::FileReader(const char * s) : m_input(), m_lastLine(), m_fileName(s) 
{
    m_input.open(m_fileName.c_str());
    if (!m_input.is_open())
        throw "Error: file not found";
